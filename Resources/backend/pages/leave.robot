...
Add Leave Type
    ...
    ${response}   POST On Session    alias=ORANGE    url=/api/v2/leave/leave-types     data=${leave_form_data}       headers=${header}    expected_status=${status}
    IF    '${status}' == '200'
...
Delete Leave Type
    ...
    ${header}     Create Dictionary header    set_cookie=${cookie}
    ${response}   DELETE On Session    alias=ORANGE    url=/api/v2/leave/leave-types     data=${leave_type_del_data}       headers=${header}   expected_status=${status}
