...
Add Job Title
    ...
    ${response}   POST On Session    alias=ORANGE    url=/api/v2/admin/job-titles     data=${job_form_data}       headers=${header}    expected_status=${status}
    IF    '${status}' == '200'
...
Delete Job Title
    ...
    ${header}     Create Dictionary header    set_cookie=${cookie}
    ${response}   DELETE On Session    alias=ORANGE    url=/api/v2/admin/job-titles     data=${job_del_data}       headers=${header}   expected_status=${status}
...
Add Location
    ...
    ${header}     Create Dictionary header    set_cookie=${cookie}
    ${response}   POST On Session    alias=ORANGE    url=/api/v2/admin/locations     data=${location_form_data}       headers=${header}    expected_status=${status}
    IF    '${status}' == '200'
        ${json_response}             evaluate             json.loads('''${response.content}''')   json
        ${data}            Get From Dictionary    ${json_response}    data
        ${id}              Get From Dictionary    ${data}    id
...
Delete Location
    ...
    ${header}     Create Dictionary header    set_cookie=${cookie}
    ${response}   DELETE On Session    alias=ORANGE    url=/api/v2/admin/locations     data=${location_del_data}       headers=${header}   expected_status=${status}
...
Change Corp Brand
    ...
    ${header}     Create Dictionary header    set_cookie=${cookie}
    ${response}   PUT On Session    alias=ORANGE    url=/api/v2/admin/theme     data=${non_default_color}       headers=${header}    expected_status=${status}
...
Activate/Deactivate Modules
    ...
    ${header}     Create Dictionary header    set_cookie=${cookie}
    ${response}   PUT On Session    alias=ORANGE    url=/api/v2/admin/modules     data=${non_default_color}       headers=${header}    expected_status=${status}
...
Change Language
    ...
    ...    
    ...    Author: Gabriel Martins
    [Arguments]    ${cookie}    ${status}   ${language}=en_US_POSIX   ${date_format}=Y-d-m   
    ${language}   Set Variable   {"language": "${language}", "dateFormat": "${date_format}"}
    ${header}     Create Dictionary header    set_cookie=${cookie}
    ${response}   PUT On Session    alias=ORANGE    url=/api/v2/admin/localization     data=${language}       headers=${header}    expected_status=${status}
...
Send Email Notification
    ...
    ${header}     Create Dictionary header    set_cookie=${cookie}
    ${response}   PUT On Session    alias=ORANGE    url=/api/v2/admin/email-configuration     data=${email_data}       headers=${header}    expected_status=${status}