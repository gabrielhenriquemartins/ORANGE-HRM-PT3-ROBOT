...
Add Employee
    ...
    ${response}   POST On Session    alias=ORANGE    url=/api/v2/pim/employees     data=${employee_form_data}       headers=${header}    expected_status=${status}
    IF    '${status}' == '200'
...
Delete Employee
    ...
    ${header}     Create Dictionary header    set_cookie=${cookie}
    ${response}   DELETE On Session    alias=ORANGE    url=/api/v2/pim/employees     data=${employee_del_data}       headers=${header}   expected_status=${status}
